 ...
 * @access public
 */
class PDEV_Fields_Control_Date extends PDEV_Fields_Control {
	...
	public function content_template( $post_id ) {
		...
		$year_field  = $this->get_year_field( "pdev_{$this->manager->name}_setting_{$this->setting}_year",   $year  );
		$month_field = $this->get_month_field( "pdev_{$this->manager->name}_setting_{$this->setting}_month", $month );
		$day_field   = $this->get_day_field( "pdev_{$this->manager->name}_setting_{$this->setting}_day",     $day   ); ?>
		...
		<label>
			...
			printf( __( '%1$s %2$s, %3$s', 'plugin-designer' ), $month_field, $day_field, $year_field ); ?>
	...
	public function get_year_field( $name, $value ) {
		...
		return sprintf(
			...
			esc_html__( 'Year', 'plugin-designer' ),
			esc_attr( $name ),
			esc_attr( $value ),
			esc_attr( date( 'Y' ) )
		);
	...
	public function get_month_field( $name, $value ) {
		...
		for ( $i = 1; $i < 13; $i = $i +1 ) {
			...
			$options .= sprintf(
				...
				sprintf( esc_html__( '%1$s-%2$s', 'plugin-designer' ), $monthnum, $monthtext )
			);
		}

		return sprintf(
			'<label><span class="screen-reader-text">%s</span><select name="%s">%s</select></label>',
			esc_html__( 'Month', 'plugin-designer' ),
			esc_attr( $name ),
			$options
		);
	}
	...
	public function get_day_field( $name, $value ) {
		...
		return sprintf(
			...
			esc_html__( 'Day', 'plugin-designer' ),
			esc_attr( $name ),
			esc_attr( $value ),
			esc_attr( date( 'd' ) )
		);
