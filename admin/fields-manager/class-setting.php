...
class PDEV_Fields_Setting {
	...
	public function __construct( $manager, $name, $args = array() ) {
		...
		if ( $this->sanitize_callback )
			add_filter( "pdev_{$this->manager->name}_sanitize_{$this->name}", $this->sanitize_callback, 10, 2 );
	}
	...
	public function get_value( $post_id ) {
		...
		return pdev_get_plugin_meta( $post_id, $this->name );
	}
	...
	public function get_posted_value() {
		...
		if ( isset( $_POST[ "pdev_{$this->manager->name}_setting_{$this->name}" ] ) )
			$value = $_POST[ "pdev_{$this->manager->name}_setting_{$this->name}" ];
...
//wp_die( var_dump( $new_value !== $old_value ) );
		...
		if ( ! $new_value && $old_value )
			pdev_delete_plugin_meta( $post_id, $this->name );

		// If the new value doesn't match the old value, set it.
		else if ( $new_value !== $old_value )
			pdev_set_plugin_meta( $post_id, $this->name, $new_value );
	}
}
