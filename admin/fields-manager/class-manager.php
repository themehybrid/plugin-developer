...
class PDEV_Fields_Manager {
	...
	public function __construct( $name ) {
		...
		require_once( 'class-control-multi-avatars.php' );
		require_once( 'class-setting-array.php' );
	...
	private function register() {
		...
		do_action( "pdev_{$this->name}_manager_register", $this );
	...
	public function register_section( $section, $args = array() ) {
		...
		if ( ! is_object( $section ) )
			$section = new PDEV_Fields_Section( $this, $section, $args );
	...
	public function register_control( $control, $args = array() ) {
		...
		if ( ! is_object( $control ) )
			$control = new PDEV_Fields_Control( $this, $control, $args );
	...
	public function register_setting( $setting, $args = array() ) {
		...
		if ( ! is_object( $setting ) )
			$setting = new PDEV_Fields_Setting( $this, $setting, $args );
	...
	public function display( $post_id ) {
		...
		$id    = sanitize_html_class( "pdev-fields-manager-{$this->name}" );
		$class = 'pdev-fields-manager ' . sanitize_html_class( "pdev-fields-manager-{$this->name}" ); ?>

		<div id="<?php echo $id; ?>" class="<?php echo $class; ?>">
			<?php $this->nav( $post_id ); ?>
			<?php $this->content( $post_id ); ?>
		</div><!-- .pdev-fields-manager -->
	<?php }
	...
	public function nav() { ?>
		...
		<?php foreach ( $this->sections as $section ) : ?>
			...
			<li>
				<a href="<?php echo esc_attr( "#pdev-fields-section-{$section->name}" ); ?>"><i class="<?php echo $icon; ?>"></i> <span class="label"><?php echo esc_html( $section->label ); ?></span></a>
			</li>
	...
	public function content( $post_id ) {
		...
		wp_nonce_field( "pdev_{$this->name}_nonce", "pdev_{$this->name}" ); ?>

		<div class="pdev-fields-content">
		...
		<?php foreach ( $this->sections as $section ) : ?>
			...
			<div id="<?php echo esc_attr( "pdev-fields-section-{$section->name}" ); ?>" class="pdev-fields-section <?php echo esc_attr( "pdev-fields-section-{$section->name}" ); ?>">

				<?php if ( $section->description ) : ?>
					<p class="pdev-fields-description description"><?php echo $section->description; ?></p>
				<?php endif; ?>
				...
				<?php foreach ( $this->controls as $control ) : ?>
					...
					<?php if ( $section->name === $control->section ) : ?>
						...
						<div id="<?php echo esc_attr( "pdev-fields-control-{$control->name}" ); ?>" class="pdev-fields-control <?php echo esc_attr( "pdev-fields-control-type-{$control->type}" ); ?>">
							<?php $control->content_template( $post_id ); ?>
						</div><!-- .pdev-fields-control -->

					<?php endif; ?>

				<?php endforeach; ?>

			</div><!-- .pdev-fields-section -->

		<?php endforeach; ?>

		</div><!-- .pdev-fields-content -->
	<?php }
	...
	public function update( $post_id ) {
		...
		// Verify the nonce.
		if ( ! isset( $_POST["pdev_{$this->name}"] ) || ! wp_verify_nonce( $_POST["pdev_{$this->name}"], "pdev_{$this->name}_nonce" ) )
			return;
